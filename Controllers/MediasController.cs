...
public class MediasController : Controller
{
    ...
    private List<Media> _getItems(int index, int nbItems)
    {
        ...
        DB.Likes.Init(Server.MapPath("~/App_Data/Likes.json"));
                ...
                switch ((MediaSortBy)Session["MediaSortBy"])
                {
                    ...
                    case MediaSortBy.Likes:
                        result = result.OrderBy(c =>
                            DAL.DB.Likes.ToList().Count(l => l.MediaId == c.Id)
                        );
                        break;
                }
            }
    ...
    public List<UserMediaInfo> GetUsersWithMedia()
    {
        return DB.Medias.ToList()
            .GroupBy(m => m.OwnerId)
            .Select(g => {
                var u = DB.Users.Get(g.Key);
                return new UserMediaInfo
                {
                    UserId = g.Key,
                    Name = u != null ? u.Name : "(deleted user)",
                    Count = g.Count()
                };
            })
            .OrderBy(u => u.Name)
    ...
    public ActionResult SetUserFilter(int? userId)
    {
        ResetMediasPaging();
        Session["SelectedUserId"] = userId ?? 0;
    ...
    {
        {
            try
            {
                if ( DB.Likes.HasChanged || DB.Users.HasChanged || DB.Medias.HasChanged || forceRefresh)
                {
                    InitSessionVariables();
                    int pageNum = (int)Session["pageNum"];
                    int pageSize = (int)Session["pageSize"];
    ...
    [HttpPost]
    public JsonResult ToggleLike(int mediaId)
    {
        ...
        var existingLike = DB.Likes.ToList()
            .FirstOrDefault(l => l.MediaId == mediaId && l.UserId == user.Id);
        ...
        var likeCount = DB.Likes.ToList()
            .Count(l => l.MediaId == mediaId);

        var users = DB.Likes.ToList()
            .Where(l => l.MediaId == mediaId)
            .Select(l => {
                var u = DB.Users.Get(l.UserId);
                return u != null ? u.Name : "(deleted user)";
            })
            .ToList();
