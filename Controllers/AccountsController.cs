        ...
        public ActionResult DeleteUser(int id)
        {
            ...
            if (id != 1)
            {
                ...
                if (user != null)
                {
                    ...
                    var likesToDelete = DB.Likes.ToList()
                        .Where(l => l.UserId == id)
                        .ToList();
                    ...
                    var mediasToDelete = DB.Medias.ToList()
                    ...
                    var orphanMedias = DB.Medias.ToList()
                        .Where(m => DB.Users.Get(m.OwnerId) == null)
                        .ToList();

                    foreach (var media in orphanMedias)
                        DB.Medias.Delete(media.Id);
                    DB.Users.Delete(id);
                    AccountsEmailing.SendEmailUserStatusChanged(message, user);
